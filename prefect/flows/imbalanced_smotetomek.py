...
def smotetomek_oversampling(X_train, y_train, image_size):
    ...
    X_train_smotetomek, y_train_smotetomek = smt.fit_resample(
...
def imbalanced_smotetomek_pipeline():
    image_size = 256

    mlflow.set_tracking_uri(uri="http://mlflow.mlflow.svc.cluster.local:5000")
    mlflow.set_experiment("Pneumonia Balance Data - Imbalanced SMOTETomek")
    ...
    X_train_smotetomek, y_train_smotetomek = smotetomek_oversampling(X_train, y_train, image_size)
    print(f'After ADASYN: {Counter(y_train_smotetomek)}')

    with mlflow.start_run():
        model_design_cnn = create_cnn_model(image_size)
        model_trained = train_model(X_train_smotetomek, y_train_smotetomek, X_val, y_val, model_design_cnn, 32, 5)
        predict_model(X_test, y_test, model_trained)


if __name__ == "__main__":
    with tags("local"):
        imbalanced_smotetomek_pipeline()
