...
def create_cnn_model(image_size):
...
def train_model(X_train, y_train, X_val, y_val, model, batch_size, epochs):
    ...
    mlflow.set_experiment("Pneumonia Imbalanced Data - SMOTE")
...
def imbalanced_smote_pipeline():
    ...
    X_train_smote, y_train_smote = smote_oversampling(X_train_imbalanced, y_train_imbalanced, image_size)
    model_design_cnn = create_cnn_model(image_size)
    model_trained = train_model(X_train_smote, y_train_smote, X_val, y_val, model_design_cnn, 32, 10)
