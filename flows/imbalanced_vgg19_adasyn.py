...
def adasyn_oversampling(X_train, y_train, image_size):
    ...
    X_train_adasyn, y_train_adasyn = smt.fit_resample(X_train, y_train)
...
def train_model(X_train, y_train, X_val, y_val, model, batch_size, epochs):
    ...
    mlflow.set_experiment("Pneumonia Imbalanced Data - VGG19 ADASYN")
...
def imbalanced_vgg19_adasyn_pipeline():
    ...
    X_train_adasyn, y_train_adasyn = adasyn_oversampling(X_train_imbalanced, y_train_imbalanced, image_size)
    model_design_cnn = create_vgg19_model(image_size)
    model_trained = train_model(X_train_adasyn, y_train_adasyn, X_val, y_val, model_design_cnn, 32, 10)
